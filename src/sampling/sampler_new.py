...
class _AlphaPEMRunner:
    ...
    def run_one(self, sample: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "meta": {
                "n_points": nb_loads,
                "delta_pola": tuple(p["delta_pola"]),
                "i_max_pola": float(i_max_pola),
                "type_plot": params["type_plot"],
            },
        }
...
def run_alphapem_from_df(
    ...
    *,
    ...
    total = len(df)
    print(f"Running AlphaPEM on {total} configuration(s)...")
    print(f"Directory for results → {save_path}")
    print(f"Directory for errors   → {errors_path}")
    ...
    for i, (idx, row) in enumerate(df.iterrows(), start=1):
        ...
        if (i % save_every == 0):
            ...
            print(f"Checkpoint: {i}/{total} → {save_path}")
    ...
    _write_errors(errors, errors_path)
    if errors:
        print(f"Logged {len(errors)} error(s) → {errors_path}")
