...
def get_polarisation_curve_samples_modified(sampled_parameters, fixed_parameters, save_path="../data/raw/results.pkl", save_every=10):
    results = []

    for i, sample in enumerate(sampled_parameters):
        try:
            combined_parameters = {**sample, **fixed_parameters}
            Simulator = AlphaPEM_modified(**combined_parameters)
            ...
            Ueq, eta_c_,i_n_ = np.array(variables['Ueq']),np.array(variables['eta_c_']),np.array(variables['i_n_'])
            i_fc_list, Rp_list = np.array(variables['i_fc_list']),np.array(variables['Rp_list'])
            ...
            if type_plot == "fixed":
                ...
                #....................................
                Ueq_discretized = np.zeros(nb_loads)
                eta_c__discretized = np.zeros(nb_loads)
                i_fc_list_discretized = np.zeros(nb_loads)
                i_n__list_discretized = np.zeros(nb_loads)
                Rp_list_discretized = np.zeros(nb_loads)
                ...
                for k in range(nb_loads):
                    ...
                    #....................................
                    Ueq_discretized[k] = Ueq[idx]
                    eta_c__discretized[k] = eta_c_[idx]
                    i_fc_list_discretized[k] = i_fc_list[idx]
                    i_n__list_discretized[k] = i_n_[idx]
                    Rp_list_discretized[k] = Rp_list[idx]
                ...
                #.............................
                combined_parameters['Ueq'] = Ueq_discretized
                combined_parameters['eta_c_'] = eta_c__discretized
                combined_parameters['i_fc_'] = i_fc_list_discretized
                combined_parameters['i_n_'] = i_n__list_discretized
                combined_parameters['Rp'] = Rp_list_discretized
